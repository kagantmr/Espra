...
static WINDOW* login_window(void) {
    int sy = (LINES - LOGIN_H) / 2; if (sy < 0) sy = 0;
    int sx = (COLS  - LOGIN_W) / 2; if (sx < 0) sx = 0;
    ...
    int lx = 1 + (LOGIN_W - 2 - LOGO_W) / 2; if (lx < 1) lx = 1;
...
int main(void) {
    ...
    if (serv_hdr.flags & ESPRA_ERROR) {
        ...
        switch ((uint8_t)reply_buf[0]) {
            case AUTH_ERR_PWD:   reason = "Wrong server password.";     break;
            case AUTH_ERR_TAKEN: reason = "Username already in use.";   break;
            case AUTH_ERR_FULL:  reason = "Server is full (max 32).";   break;
            default:             reason = "Unknown auth error.";        break;
        }
